...
> ## ✅ สถานะตอนนี้: ชุดแรกครบแล้ว ด้วยไฟล์ CC0 จาก Kenney
>
> **มีแล้ว 23 เสียง** หมัดเบา 5 · หมัดหนัก 4 · บล็อก 3 · ลงพื้นเบา 3 · ลงพื้นแรง 3 ·
> หวดลม 3 · แส้ 2 — ทั้งหมด CC0 ใช้เชิงพาณิชย์ได้ ไม่บังคับเครดิต
> รายละเอียดว่าเลือกไฟล์ไหนเพราะอะไร อยู่ใน [`docs/AUDIO.md`](game/docs/AUDIO.md)
>
> **ที่ยังขาด และ Kenney ไม่มีให้** — ต้องเจนหรือหาจากที่อื่น:
> ปืนไรเฟิล · ปืนสั้น · ระเบิด · ไฟติด · ไฟลุก (วนได้) · หายตัว/ปรากฏ · หมอกฝุ่น ·
> กล่องผี · สลับอาวุธ · ชนกำแพง · ม้วนตัว · โดดสองชั้น · เกราะ · อัลติ · น็อกเอาต์ · ปุ่มเมนู
>
> **⚠️ ก่อนเจนเพิ่มจาก ElevenLabs อ่านข้อ 7 ก่อน** — ตอนนี้เกมใช้เชิงพาณิชย์ได้ทั้งหมด
> ใส่ไฟล์จากแพ็กฟรีกลับเข้ามา**ไฟล์เดียว** ทั้งเกมกลับไปติดข้อห้ามเชิงพาณิชย์ทันที
> (`SOURCES.json` กับเทสต์จะจับได้เอง แต่รู้ไว้ก่อนตัดสินใจดีกว่า)
...
> ✅ **ทำแล้ว 2 เวอร์ชัน** อยู่ในเกมแล้ว — `hit_light_1` (84 ms บางและแหลม)
> และ `hit_light_2` (137 ms มีเนื้อกว่า) ยังเติมได้อีก 1-2 อันถ้าอยากให้หลากหลายขึ้น
>
> **⚠️ สองไฟล์จากรอบนั้นเจนพลาด** `Sharp_Strike` กับ `The_Sudden_Hit`
> เป็นไฟล์ mp3 ที่ถูกต้องทุกอย่าง 6.03 วินาที 192 kbps สเตอริโอ **แต่ข้างในเงียบทั้งไฟล์**
> (−91 dB ทั้ง 532,224 ตัวอย่าง ซึ่งคือพื้นของ 16 บิต ไม่ใช่เสียงเบาแต่คือไม่มีอะไรเลย)
> เป็นอาการที่ ElevenLabs เจนพลาดแล้วยังให้ดาวน์โหลดได้ตามปกติ — **เจนใหม่อย่างเดียว**
> เปิดฟังก่อนส่งทุกครั้งจะเร็วกว่า
...
## 7 · เรื่องสิทธิ์ — ยืนยันแล้วว่าใช้แพ็กฟรี

**สองข้อที่ต้องรู้**

1. **ต้องอ้างอิง `elevenlabs.io`** เมื่อเผยแพร่ต่อสาธารณะ — **ใส่ให้แล้ว**
   อยู่ในบรรทัดเครดิตหน้าเมนู และมีเทสต์ผูกไว้กับการมีอยู่ของไฟล์เสียง
   ถ้าไฟล์ยังอยู่แต่เครดิตหาย เทสต์แดงทันที

2. **ใช้แบบไม่แสวงหากำไรเท่านั้น** — การใส่เครดิต**ไม่ได้**ปลดข้อนี้
   ตอนนี้ไม่มีปัญหา เกมเล่นฟรีบน GitHub Pages ไม่มีโฆษณา ไม่ขาย
   **แต่ถ้าวันไหนจะหาเงินจากเกมนี้ ไฟล์ชุดนี้ใช้ต่อไม่ได้** ต้องเจนใหม่บนแพ็กเสียเงิน
   (แพ็กถูกสุดที่ให้สิทธิ์เชิงพาณิชย์คือ Starter) หรือเปลี่ยนไปใช้แหล่งอื่น

> เปิดหน้า Terms ของ ElevenLabs โดยตรงไม่ได้ (โดเมนถูกบล็อกจาก proxy ของเครื่องที่รันอยู่)
> ข้อมูลนี้มาจากแหล่งรองที่ตรงกันหลายแหล่ง ถ้าจะพึ่งพาจริงจังควรเปิดอ่านเองอีกรอบ

**ทางเลือกถ้าไม่อยากติดเงื่อนไขนี้ตั้งแต่แรก:** freesound.org มีไฟล์ CC0 (ใช้ได้ทุกอย่าง
ไม่ต้องให้เครดิตด้วยซ้ำ) แต่ต้องเช็คสัญญาอนุญาตเป็นไฟล์ ๆ ไม่ใช่เหมารวมทั้งเว็บ
เพราะในเว็บเดียวกันมีทั้ง CC0, CC-BY (ต้องให้เครดิต) และ CC-BY-NC (ห้ามเชิงพาณิชย์) ปนกัน

---

## 8 · ทางเลือก CC0 — ถ้าอยากหลุดจากเงื่อนไขทั้งสองข้อ

**CC0 คือ "สละสิทธิ์ทั้งหมด"** ใช้เชิงพาณิชย์ได้ ดัดแปลงได้ ขายต่อได้ และ**ไม่ต้องให้เครดิตด้วยซ้ำ**
(ให้ก็ได้ เป็นมารยาท แต่ไม่ใช่เงื่อนไข) เป็นสัญญาอนุญาตที่สบายที่สุดที่มี

### สามแหล่งที่ควรดู เรียงตามความคุ้มแรง

**1 · Kenney — <https://kenney.nl/assets/impact-sounds>** ← เริ่มที่นี่
ชุด **Impact Sounds มี 130 เสียง CC0 ทั้งหมด** จัดกลุ่มตามวัสดุที่กระแทก
ดาวน์โหลดเป็นไฟล์เดียวจบ ไม่ต้องสมัครสมาชิก ไม่ต้องไล่เช็คสัญญาอนุญาตรายไฟล์
ชุด <https://kenney.nl/assets/interface-sounds> และ <https://kenney.nl/assets/ui-audio>
ใช้กับปุ่มในเมนูได้เลย (ข้อ 5 ในไฟล์นี้)

> **ข้อควรรู้ก่อนโหลด:** เสียงของ Kenney เป็นแนวอาร์เคดที่สะอาดและ "บาง" กว่า
> ที่เจนจาก ElevenLabs ค่อนข้างชัด เข้ากับเพลงชิปทูนดีมาก แต่ถ้าอยากได้หมัดหนัก
> แบบมีเนื้อ อาจต้องซ้อนสองเสียงหรือหาจากแหล่งอื่นเสริม
> ต้องเปิดฟังเองถึงจะรู้ว่าชอบไหม — ตรงนี้ผมตัดสินใจแทนไม่ได้

**2 · freesound.org** — ใหญ่ที่สุดและสมจริงที่สุด แต่มีขั้นตอนมากกว่า
ต้องสมัครสมาชิกถึงจะโหลดได้ และ**ต้องกรองสัญญาอนุญาตเอง** ในเว็บเดียวกันมีทั้ง
CC0 (สบายสุด) · CC-BY (ต้องเครดิตรายไฟล์) · CC-BY-NC (ห้ามเชิงพาณิชย์ — กลับไปที่เดิม)
ใช้ตัวกรองในหน้าค้นหาเลือกเฉพาะ CC0 แล้วจะเหลือของที่ใช้ได้โดยไม่ต้องคิดอะไรอีก

**3 · Sonniss "GDC Game Audio Bundle"** — ชุดเสียงระดับมืออาชีพที่แจกฟรีทุกปี
คุณภาพสูงกว่าสองอันบนมาก ใช้เชิงพาณิชย์ได้ **แต่ไม่ใช่ CC0** เป็น royalty-free
ที่ห้ามขายต่อเป็นไฟล์เสียง (ซึ่งเราไม่ได้จะทำอยู่แล้ว) ข้อเสียคือไฟล์ใหญ่หลายสิบกิกะไบต์
และต้องนั่งฟังคัดเอง เหมาะถ้าจริงจังเรื่องเสียงมาก ๆ

### ขั้นตอนเหมือนเดิมทุกอย่าง

โหลดมาแล้ว**โยนไฟล์มาได้เลย บอกว่าอันไหนคืออะไรและมาจากแหล่งไหน**
`build_sfx.py` รับทุกฟอร์แมตที่ ffmpeg อ่านออก (wav / ogg / mp3 / flac)
และบันทึกสัญญาอนุญาตต่อไฟล์ให้เอง — ผสมหลายแหล่งในโฟลเดอร์เดียวกันได้ ไม่มีปัญหา

**จะเปลี่ยนทีหลังก็ได้ ไม่ต้องรีบตัดสินใจตอนนี้** ระบบไม่ผูกกับ ElevenLabs เลย
ต้นทุนการเปลี่ยนคือแก้ `MAP` ในสคริปต์แล้วรันใหม่ เท่านั้น

### ทำไมผมโหลดมาคัดให้เองไม่ได้

เครื่องที่ผมรันอยู่ถูกตั้งค่าให้ปิดการเชื่อมต่อออกนอก ทุกโดเมนข้างบนถูกปฏิเสธหมด
(รวมถึง elevenlabs.io ด้วย) ถ้าอยากให้ผมโหลดและคัดเสียงให้โดยตรงจริง ๆ
ต้องเปิดสิทธิ์เข้าถึงเครือข่ายให้ก่อน — ตั้งได้ที่เมนู environment ของ session แล้วกด Edit
จะเปิดกว้างขึ้นทั้งระดับ หรือใส่เฉพาะโดเมนที่ต้องใช้ก็ได้
รายละเอียดระดับการเข้าถึงอยู่ที่ <https://code.claude.com/docs/en/claude-code-on-the-web>
