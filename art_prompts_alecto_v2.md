# ALECTO v2 — prompt ชีตทั้งชุด (style anchor ใหม่ ต่อให้ในทุกบล็อกแล้ว)

> ✅ **ขั้น 0 ผ่าน** — `art_reference/alecto_idle_APPROVED_v2.jpg` **แนบภาพนี้ไปกับทุก prompt**

> 🔫 **ไรเฟิลต้องสะพายหลังทุกท่า** ปลายกระบอกกับพานท้ายโผล่พ้นไหล่ — เป็นของใหม่ที่ของเดิมไม่มี
> ถ้าหายไปในชีตไหน ชีตนั้นใช้ไม่ได้ เพราะในเกมเธอสลับอาวุธได้และต้องอ่านออกตลอดเวลา
>
> 🎩 **หมวกปีกแคบและดันไปหลัง** ห้ามกลับไปเป็นปีกกว้างบังหน้าแบบเดิม
> นั่นคือสาเหตุที่ไม้บรรทัดวัดสัดส่วนพังสองรอบ
>
> 🔥 **ห้ามวาดไฟจริง** รอยไหม้ที่ชายเสื้อคือรอยไหม้ ไม่ใช่เปลวไฟ เอฟเฟคเกมวาดเอง
...
**ต่อให้ในทุกบล็อกแล้ว ไม่ต้องวางเอง** (ของเดิมต้องวางท้ายเอง ซึ่งลืมง่าย)
ถ้าต้องแก้ แก้ที่บล็อกนั้น ๆ โดยตรง
...
### ✅ ผ่านแล้ว — `art_reference/alecto_idle_APPROVED.jpg`

**แนบไฟล์นี้กับทุกชีตต่อจากนี้** สไตล์จะได้ไม่ไหลระหว่างชีต

#### ไม้บรรทัดเดิม (ตัวสูง ÷ หัวกว้าง) ใช้กับตัวนี้ไม่ได้

วัดได้ **3.01** ซึ่งเกินช่วง 2.50–2.80 แต่**ไม่ได้แปลว่าไม่ผ่าน** — ไม้บรรทัดนี้ตั้งมาจากตัวที่ไม่สวมหมวก
หมวกบังกะโหลกจนวัดความกว้างหัวจริงไม่ได้ บวกกับหน้าเธอทรงเรียวกว่าสองตัวแรกโดยดีไซน์

ใช้สองไม้บรรทัดนี้แทน ซึ่งหมวกไม่เกี่ยวเลย:

| | คางถึงพื้นรองเท้า | หัว+หมวก เหนือคาง |
|---|---|---|
| Nyx | 62% | 38% |
| Helios | 62% | 38% |
| **Alecto** | **64%** | **36%** |

ต่างกัน 2% ทั้งสองช่อง = สัดส่วนลำตัวตรงกันแทบเป๊ะ
วางเทียบกันโดยสเกลจากคางถึงเท้าแล้วอยู่ในภาษาเดียวกัน (ดู `docs/ALECTO_KIT.md`)

**ตอนประกอบเข้าเกมต้องวัดสเกลจากคางถึงเท้า ไม่ใช่ความสูงทั้งภาพ**
ถ้าใช้ความสูงทั้งภาพ หมวกจะถูกนับเป็นส่วนหนึ่งของตัว แล้วลำตัวเธอจะถูกย่อจนเตี้ยกว่าอีกสองคน

#### สิ่งเดียวที่ต่างจริง

เธอ**ผอมกว่า** ทั้งแขน ขา และลำตัว — เป็นเรื่องดีไซน์ตัวละคร ไม่ใช่สไตล์หลุด
(นักแม่นปืนสาวร่างเพรียว เทียบกับนักสู้ล่ำสองคน) ปล่อยไว้แบบนี้ได้ ไม่ต้องเจนใหม่

ถ้าอยากให้กลืนกว่านี้อีก เจนรอบเดียวลองใส่ *"rounder wider face, chunkier arms and legs"*
แล้วเทียบกัน ถ้าไม่ดีขึ้นใช้ของเดิม — **ห้ามลองเกินหนึ่งรอบ** ของที่มีอยู่ใช้ได้แล้ว

---

---

## ขั้น 1 — คลิป ยืน → วิ่ง — ✅ เสร็จแล้ว

เฟรมที่ตัดไว้อยู่ที่ **`art_reference/alecto_clip_strip.png`** (ช่อง 0 = ท่ายืน · ช่อง 1-10 = วิ่งหนึ่งรอบ)
ตัวเลขที่วัดได้อยู่ใน `alecto_clip_strip.json`

คลิป 24fps 192 เฟรม · ยืนถึงเฟรม ~45 แล้ววิ่ง · ช่วงที่วิ่งนิ่งแล้วคือเฟรม 105 เป็นต้นไป

**รอบเต็มคือ 21 เฟรม ไม่ใช่ 10** — เทียบภาพทีละเฟรมแล้วเฟรม 105 กับ 126 เหมือนกันที่สุด (IoU 0.95)
ส่วนเฟรม 115 ที่ระยะถ่างขาเท่ากันนั้น**ขาคนละข้าง** ถ้าตัดรอบละ 10 เฟรมตรง ๆ จะได้ท่าที่
นำด้วยขาข้างเดิมตลอด เห็นเป็นรอยสะดุดตอนวน จึงเอาเฟรมเว้นเฟรมจากรอบ 21 เฟรม เหลือ 10 เฟรม

| วัดได้ | ค่า |
|---|---|
| ท่ายืนในคลิป สูงรวมหมวก | 667 px |
| คางถึงพื้นรองเท้า | 424 px (**64%** — ตรงกับภาพที่อนุมัติเป๊ะ) |
| ระยะถ่างขาสูงสุด | 451 px |
| **`runStride` ที่ควรใช้** | **86** (Nyx 102 · Helios 85) |
| ความสูงสไปรท์ที่จะได้ | 127 px (Nyx/Helios = 130) |

หมวกไม่ได้ทำให้เธอสูงเกินอย่างที่กลัวไว้ — ยึดคางถึงเท้าแล้วได้ 127 ต่างจาก 130 แค่ 2%

---

## ขั้น 1 (เดิม) — prompt คลิป เก็บไว้เผื่อเจนซ่อม
...
Any crouching or ducking pose must be crouched LOW — total height only about 80% of the standing pose, clearly shorter, not a shallow knee-bend.

Chibi-proportioned anime game sprite, large head roughly one third of the total height, short limbs, bold dark outlines, flat cel shading, muted desaturated palette — match the attached reference proportions exactly. Character: a young woman with long deep-red hair in a single thick braid over one shoulder with darker ember-red highlights. She wears a NARROW-brimmed dark red hat pushed back off her forehead so her hairline and forehead stay clearly visible, with a leather band, brass goggles and small turquoise stones on it. A near-black burgundy leather vest over a bright cream long-sleeved shirt with rolled sleeves, a turquoise neckerchief at her throat, and a leather bandolier of small corked glass bottles across her chest. A wide brown belt with a silver and turquoise buckle and a tooled revolver holster. Dark brown fringed leather chaps over navy jeans and scuffed brown boots, with the lower hem of the chaps and vest scorched with dark burn marks. A lever-action rifle is slung diagonally across her back, its barrel and stock visible past her shoulders IN EVERY POSE. Fingerless dark brown leather gloves. Three-quarter view, body angled toward the viewer's right. Pure white background, no shadow, no ground line, no props beyond those described, no text, no labels, no panel borders. Full body visible from the top of the hat to the soles of both boots — do not crop, do not zoom. Identical camera distance and identical character size in every pose. Exactly two arms and two legs, clearly separated, do not overlap or duplicate limbs. Keep the whip and all fringe close to her body, never stretching more than one body-width away. No effects of any kind — no fire, no smoke, no sparks, no glow, no motion lines.
```
...
## ชีต H — ถอยหลัง/กลิ้ง (6 ท่า · 2 แถวแถวละ 3) · ✅ เจนแล้ว เข้าเกมแล้ว
...
ตอนแรกตั้งใจใช้ท่ากระโดดกับท่ากลิ้งของชีต A ไปก่อน แต่พอมีชีตเฉพาะแล้วดีกว่าชัดเจน
โดยเฉพาะท่าที่ "มือเอื้อมไปที่เข็มขัด" ซึ่งเป็นเฟรมที่เกมสลับอาวุธพอดี

### บันทึกไว้: ทางที่เคยคิดจะใช้ของฟรี
