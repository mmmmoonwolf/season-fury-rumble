...
// ซึ่งอ่านเป็นฉากแบนไถล ความลึกเกิดจากความต่างของอัตรา ไม่ใช่จากการขยับ
{
  ...
  // ตัวขับคือตำแหน่งเฉลี่ยของ "ทุกคน" ไม่ใช่กล้อง — เกมนี้กล้องนิ่งสนิท
  // เฉลี่ยทั้ง roster เพราะ 2v2 ถ้าคิดจากสองคนแรก ฉากหลังจะเลื่อนสวนกับคนที่สามสี่
  ok(/s\.fighters\.reduce\(\(a, f\) => a \+ f\.x, 0\) \/ s\.fighters\.length - STAGE\.w \/ 2/.test(scene),
    "แกนนอนขับด้วยจุดกึ่งกลางของทุกคน");
  // แกนตั้งต้องคิดจาก "สูงกว่าพื้นเท่าไหร่" และตัดค่าติดลบทิ้ง
  // ถ้าเลื่อนขึ้นเหนือเส้นฐานได้ ใต้หน้าผาจะโหว่ให้เห็นฟ้า
  ok(/Math\.max\(0, STAGE\.groundY - s\.fighters\.reduce\(\(a, f\) => a \+ f\.y, 0\) \/ s\.fighters\.length\)/.test(scene),
