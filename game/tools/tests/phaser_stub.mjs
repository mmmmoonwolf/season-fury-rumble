...
export function makeScene(){
  ...
  const scene = {
    ...
    add:{ existing(){}, sprite(x,y,tex,fr){ const sp=new Sprite(null,x,y,tex); sp.frame={name:fr}; sp.displayHeight=0; sp.destroy=()=>{sp.dead=true}; sp.setFlipX=(f)=>{sp.flipX=f;return sp}; log.push('sprite:'+tex); return sp },
      ...
      rectangle(x,y,w,h,c,a2){ const r={x,y,width:w,height:h,color:c,alpha:a2??1,setOrigin(){return r},setDepth(d){r.depth=d;return r},setScrollFactor(f){r.scrollFactor=f;return r},setStrokeStyle(){return r},destroy(){r.dead=true}}; return r },
