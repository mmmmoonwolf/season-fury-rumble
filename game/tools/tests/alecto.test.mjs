...
// ── อัลติขังคู่ต่อสู้ไว้ในหมอก ──
//
// วงเดียวทำสองหน้าที่: ที่กำบังของเธอ และกรงของอีกฝ่าย
// เธอเดินทะลุออกไปได้ (veil) อีกฝ่ายเดินออกทางข้างไม่ได้ ต้องกระโดดข้ามขอบ
// นั่นคือช่วงเวลาที่เธอใช้หนีจริง ๆ ไม่ใช่แค่ "หายไปแป๊บหนึ่งแล้วโดนไล่ต่อ"
{
  // คู่ต่อสู้ยืนอยู่กลางวง แล้วเดินออกด้านข้างตลอด
  const walkOut = (withDust) => {
    const g = mk(60);
    g.p2.x = g.p1.x;                       // ยืนซ้อนกันกลางวง
    if (withDust) g.dust = { x: g.p1.x, owner: "p1", life: 400 };
    const x0 = g.p2.x;
    for (let i = 0; i < 120; i++) g.step(inp(), inp({ right: 1 }));
    return Math.round(g.p2.x - x0);
  };
  const free = walkOut(false), caged = walkOut(true);
  ok(free > 300, `ไม่มีวง: เดินออกไปได้ ${free} px`);
  ok(caged <= 205, `มีวง: เดินได้ไกลสุดแค่ขอบวง (${caged} px · ขอบอยู่ที่ 200)`);
}

// ── แต่กระโดดข้ามขอบออกไปได้ ──
// ถ้าออกไม่ได้เลย อัลติจะเป็น "เสียเทิร์นไปเฉย ๆ" ซึ่งไม่มีทางสวน
// ทางออกต้องมีและต้องจ่ายราคา — ราคาคือกระโดด ซึ่งอ่านออกและสวนได้
{
  const g = mk(60);
  g.p2.x = g.p1.x;
  g.dust = { x: g.p1.x, owner: "p1", life: 400 };
  const x0 = g.p2.x;
  for (let i = 0; i < 120; i++)
    g.step(inp(), inp({ right: 1, ...(i % 40 === 0 ? { jump: 1, p: { jump: 1 } } : {}) }));
  ok(g.p2.x - x0 > 260, `กระโดดข้ามขอบออกไปได้ (${Math.round(g.p2.x - x0)} px)`);
  ok(g.p2.caged === 0, "ออกไปแล้วไม่ติดกรงอีก");
}

// ── เจ้าของวงไม่ติดกรงของตัวเอง ──
{
  const g = mk(150);
  g.dust = { x: g.p1.x, owner: "p1", life: 400 };
  const x0 = g.p1.x;
  for (let i = 0; i < 120; i++) g.step(inp({ right: 1 }), inp());
  ok(g.p1.caged === 0, "เธอไม่เคยติดกรงของตัวเอง");
  ok(g.p1.x - x0 > 300, `และเดินออกจากวงตัวเองได้ตามปกติ (${Math.round(g.p1.x - x0)} px)`);
}

// ── กรงเป็นประตูทางเดียว: เดินเข้าได้ ──
//
// ถ้ากันทั้งสองทาง วงจะกลายเป็นกำแพงกันตัวเธอเอง ซึ่งไม่ใช่ท่านี้เลย
// คนที่ยืนอยู่นอกวงตั้งแต่แรกต้องไม่ติดอะไร จนกว่าจะเดินเข้ามาเอง
{
  const g = mk(150);
  g.dust = { x: g.p1.x, owner: "p1", life: 500 };
  g.p2.x = g.p1.x + 400;                   // ยืนอยู่นอกวง (ขอบอยู่ที่ +200)
  g.step(inp(), inp());
  ok(g.p2.caged === 0, "ยืนอยู่นอกวง ไม่ติดอะไร");
  for (let i = 0; i < 90; i++) g.step(inp(), inp({ left: 1 }));
  ok(g.p2.x <= g.p1.x + 200 + 1, `เดินเข้าวงได้ไม่มีอะไรกั้น (ตอนนี้อยู่ห่าง ${Math.round(g.p2.x - g.p1.x)} px)`);
  ok(g.p2.caged === 1, "แล้วติดกรงทันทีที่เข้ามา");
}

// ── วงหมดอายุแล้วกรงต้องหายไปด้วย ──
{
  const g = mk(60);
  g.p2.x = g.p1.x;
  g.dust = { x: g.p1.x, owner: "p1", life: 30 };
  for (let i = 0; i < 40; i++) g.step(inp(), inp({ right: 1 }));
  ok(g.dust === null && g.p2.caged === 0, "วงหายแล้วกรงหายตาม");
  const x0 = g.p2.x;
  for (let i = 0; i < 60; i++) g.step(inp(), inp({ right: 1 }));
  ok(g.p2.x - x0 > 150, `เดินต่อได้อิสระ (${Math.round(g.p2.x - x0)} px)`);
}
