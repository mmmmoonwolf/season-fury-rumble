...
def add_outline(img, px=3, color=(18, 16, 22), alpha_thr=110):
    """
    ...
    px ค่าเริ่มต้น 3 วัดจากขนาดที่เห็นจริงบนจอ ไม่ใช่ขนาดใน atlas:
    atlas เก็บตัวละครสูง 393 px แต่บนจอ 720p วาดจริงราว 200 px (WORLD_HEIGHT 190 x ซูม 720/672)
    = ย่อราว 0.51 เท่า เส้น 3 px จึงกลายเป็นราว 1.5 px บนจอ = ราว 0.75% ของความสูงตัวละคร
    ซึ่งเป็นสัดส่วนเดียวกับเกมต่อสู้การ์ตูนที่อ้างอิง (ลองแล้ว 4 ขึ้นไปเริ่มกินรายละเอียด
    ระบายชุด/ปอยผมจนทึบติดกัน ส่วน 2 เริ่มจางเกินบนพื้นหลังที่มีลายเยอะ)
    ตัวละครที่เก็บคนละสเกลต้องปรับตามส่วน (Nyx เก็บสเกล 280 จึงใช้ 2)
