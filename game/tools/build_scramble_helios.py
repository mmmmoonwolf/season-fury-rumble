...
SEQ = {
    ...
    # ---- ชีต F: ไม้จบสามทางของ Chain Rush + เข่าพุ่ง (สกิล 2) ----
    # ท่า 5 (โน้มตัวต่ำมือแตะพื้น) ใช้เป็นจังหวะเงื้อของทั้งเตะยกคางและกวาดขา เพราะเป็นการย่อแบบเดียวกัน
    "rushEndF": ("F", [1, 2, 3], GROUND),   # เงื้อหมัด -> หมัดตรงสุดแขน -> เก็บการ์ด
    "rushEndU": ("F", [5, 6, 4], GROUND),   # ย่อต่ำ -> เตะยกคางพ้นหัว -> ตั้งหลัก
    "rushEndD": ("F", [5, 7, 4], GROUND),   # ย่อต่ำ -> กวาดขาติดพื้น -> ตั้งหลัก
    "knee":     ("F", [4, 8, 3], GROUND),   # ตั้งท่า -> เข่าชนอก -> เก็บการ์ด

    # ---- ชีต G: Hundred Hands (อัลติ) ----
    # สามจังหวะแรกวนซ้ำได้ด้วยการกดรัว จึงสลับหมัดซ้าย-ขวา-เข่าให้ดูรัวจริงตอนวน
    "hh1":   ("G", [1, 2, 3], GROUND),
    "hh2":   ("G", [3, 2, 3], GROUND),
    "hh3":   ("G", [2, 4, 2], GROUND),
    "hhEnd": ("G", [5, 6, 1], GROUND),      # เงื้อสุด -> หมัดปิด -> ลงการ์ด
}
...
SHEETS = {n: sheet_poses(f"{SHEETS_DIR}/{n}.jpg") for n in "ABCDEFG"}
